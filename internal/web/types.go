...
// Реальный источник доменов — dpi.Builtin (используется одинаково CLI и Web).
// Эта переменная — JSON-friendly адаптер с тегами для HTTP API.
var BuiltinDPIPresets = buildPresetsView()

func buildPresetsView() []DPIPreset {
	out := make([]DPIPreset, 0, len(dpi.Builtin))
	for _, p := range dpi.Builtin {
		out = append(out, DPIPreset{
			Name:        p.Name,
