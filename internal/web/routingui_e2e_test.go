...
func TestE2E_DeleteFlow(t *testing.T) {
	...
	// add 2 outbounds
	_ = do(s, authReq("POST", "/api/outbounds", types.Outbound{Tag: "a", Type: "direct"}))
	_ = do(s, authReq("POST", "/api/outbounds", types.Outbound{Tag: "b", Type: "block"}))

	// delete one
	rec := do(s, authReq("DELETE", "/api/outbounds/a", nil))
	...
	rec = do(s, authReq("DELETE", "/api/outbounds/nonexistent", nil))
...
func TestE2E_PresetMultiApply(t *testing.T) {
	...
	for _, name := range []string{"block-ads", "ru-direct", "discord-vpn", "torrents-direct"} {
		rec := do(s, authReq("POST", "/api/presets/"+name+"/apply", nil))
		if rec.Code != http.StatusOK {
