...
Скрипты `01-install.sh`, `02-policy.sh`, `03-dpi-hostlist.sh`, `04-reboot.sh`
и `99-uninstall.sh` подключают `scripts/e2e/lib.sh`:

```sh
. "$(dirname "$0")/lib.sh"
```

Оттуда приходят: разбор `KEENETIC_HOST`/`KEENETIC_USER`/`KEENETIC_KEY`,
функции `log()`/`fail()`/`pass()` и обёртка `ssh_cmd()`. `STEP` (префикс
в строках лога) по умолчанию берётся из имени вызывающего файла
(`01-install.sh` → `01-install`); скрипт может задать `STEP` явно до
`source`, тогда оно не переопределяется.

`04-reboot.sh` после подключения `lib.sh` задаёт `SSH_OPTS="-o ConnectTimeout=5"`,
чтобы `ssh_cmd` не ждал долгим дефолтным таймаутом хост, который ещё
перезагружается. `lib.sh` — не самостоятельный скрипт и напрямую не
запускается.

`00-build.sh` `lib.sh` не подключает: он работает только локально (сборка
бинаря) и не обращается к роутеру по SSH, поэтому общая преамбула ему не
подходит.
