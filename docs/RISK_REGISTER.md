...
Риск — любое условие, при реализации которого пользователь получает: потерю доступа к роутеру, утечку трафика мимо прокси, повреждение постороннего сетевого стека, отказ автостарта без уведомления, или деградацию производительности на целевом железе ([Keenetic](https://help.keenetic.com/hc/ru), 128 MB RAM, MIPS/ARM).
...
|---------|-----------------------------------------------------------------------------------|------------------------------------------------------|
| CRIT    | Необратимый ущерб: утечка данных, brick роутера, SSH lockout без recovery         | SSH lockout (Issue #1), [ipset](https://ipset.netfilter.org/) пустой → весь трафик мимо (#17) |
...
| LOW     | Неудобство: не влияет на безопасность и функциональность при нормальном сценарии  | GitHub rate-limit при install, [TPROXY](https://www.kernel.org/doc/html/latest/networking/tproxy.html) module не проверяется явно |
...
| L       | Редкие edge-cases: нестандартное железо, экзотическая конфигурация, race   | Медленный USB >60s; MIPS big-endian; [fwmark](https://www.man7.org/linux/man-pages/man7/socket.7.html) занят чужим |
...
| R009 | Память / geo       | Geo-файлы (.srs) 10–30 MB на роутере 128 MB RAM → пиковое потребление при update                                                | MED      | L          | OOM killer убивает sign-craze или [sing-box](https://sing-box.sagernet.org/)                                  | Streaming через temp-файл с `io.CopyN` (буфер 512 KB) + atomic rename; Issue #14 DONE                          | mitigated | `tasks/safety-fixes.md:123-127` (Issue #14)                             |
...
| R011 | Web UI / безопасность | Web UI без TLS и без HTTP-аутентификации (удалена в v1.6.1), POST `/api/config` открыт в LAN → MITM или злоумышленник в LAN может изменить конфигурацию | MED      | M          | Изменение прокси-конфигурации, redirect трафика на свой сервер              | LAN-only доступ + NDM INPUT DROP от WAN (порты 9090/9091/9092 закрыты с WAN через `filter/INPUT`); `MaxBytesReader(1MB)` на всех POST (Issue #7 DONE). HTTP-аутентификации нет — bcrypt-mitigation неприменим. | partial | `tasks/safety-fixes.md:57-62` (Issue #7)                                |
...
| R015 | Конкурентность     | Concurrent `--start` от init.d и manual → race на pidfile / двойное применение firewall-правил                                  | MED      | L          | Дублированные правила; второй старт получает `ErrLocked` и падает                  | `flock /opt/var/lock/sign-craze.lock` (LOCK_EX) гарантирует исключительный доступ                                | mitigated | `internal/locks/file.go`; Issue #8                                      |
...
| R029-bcrypt-MIPS-DoS | ~~bcrypt cost=12 на MIPS softfloat: login admin UI 6 c — окно для timing/DoS атак.~~ **Неактуально — auth полностью удалён в v1.6.1 (bcrypt-каркаса нет).** | LOW |
...
| R033-xray-geo-silent-fail | xray без `geoip.dat`/`geosite.dat` падал с криптичной ошибкой `failed to load geosite`. Mitigated в v1.4.0 (early-check с подсказкой `--update-geo --core xray`; `GeoAssetsDir=""` контракт = skip для preview/Validator). Расширено в v1.6.4: `--update-geo --core xray` (и автоматически `--core-install xray`) теперь сами скачивают и SHA256-верифицируют `geosite.dat`/`geoip.dat` от `Loyalsoldier/v2ray-rules-dat` в `/opt/etc/sign-craze/xray/assets` — раньше early-check только подсказывал команду, файлы пользователь искал сам | MED |
| R034-reproducibility | Build без `-buildid=` и `SOURCE_DATE_EPOCH` → разные хеши при одинаковом коде, нет supply-chain verification. Mitigated в v1.4.0 (reproducible builds + [cosign](https://www.sigstore.dev/) keyless OIDC + [SLSA](https://slsa.dev/) provenance attestation) | MED |
| R037-ruleset-url-validation | `rule_set.URL` в `routing.json` с 404 или несовпадением формата (JSON-источник под расширением `.srs`/`.mrs`, ожидавшимся как скомпилированный бинарник) валил ядро на старте без раннего предупреждения (инцидент 2026-05-12, `tasks/lessons.md`). Mitigated в v1.6.4: `--diag` чек `rule-set-urls` (HEAD/GET + магические байты формата, WARN заранее, никогда не FAIL); Routing UI `POST /api/rule_sets` отклоняет подтверждённый 404/format-mismatch на этапе добавления (400), а не на следующем `--restart` | MED |
...
- **Pre-flight перед mutating**: любая операция, изменяющая сетевое состояние ядра (apply, install, update), выполняется только после серии проверок (fwmark availability, [iptables](https://www.netfilter.org/projects/iptables/index.html) modules, disk space, TUN device). Ошибка pre-flight → отказ с понятным сообщением, никаких частичных изменений.
